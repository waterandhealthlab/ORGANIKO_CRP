---
title: "An anti-inflammatory response of an organic food intervention treatment by reducing pesticide exposures in children of Cyprus: a cluster-randomized crossover trial - Part 2"
output:
  html_document:
    toc: yes
    toc_float: yes
...
The output of the Part 1 script needs to be created (produced_data/meas1).
...
# read the output of part 1
dir_path <- getwd()
meas1 <- readRDS(paste0(dir_path, "/produced_data/meas1.rds", sep = ""))

# create dataset with variables needed for the regression analysis 
lmmdata <- subset(meas1, select = c(
  "Code", "phase", "school.x",
  "sample_no_orig", "Group",
  "age_baseline", "age_endcon", "age_endorg",
  "Sex","logadj_crp","logcrp","cnlod","adj_pb1000","pb1000","cr","logcr" 
))
...
lmmdata$logcr_S<-scale(lmmdata$logcr)
...
saveRDS(lmmdata_dates_MOD, file.path(analysis_rds_dir, "lmmdata_dates_MOD.rds"))
...
# CK: The below functions should be deleted if not used anywhere..
...
#Create a new dataframe the specific column of lmmdata_dates_MOD and keep the non na values
visua<-lmmdata_dates_MOD%>% select(ID,logadj_crp_S_TR, logcrp_S_TR,phase,DaysfromTreatment)%>%filter(!is.na(logadj_crp_S_TR))

```
## Visualization

Visualize the association between the creatinine adjusted crp measurements and the time and the non creatinine crp adjusted measurements and the days from treatment.
...
```{r plots_model_crp_cr_INT, echo=FALSE}
...
## Model requested by Associate Editor

### Log CRP ~ baseline Log CRP + DaysfromTreatment + phase + log_creatinine + (phase*DaysfromTreatment if p<0.05) + random(...)

```{r  crpmodel_INT_logcr_int, echo=FALSE, include=FALSE}

# model with interaction term
model_crp_logcr_INT <- lmer(logcrp_S_TR ~ logcrp_S_BL + DaysfromTreatment + phase + logcr_S + phase*DaysfromTreatment +(1 | ID), lmmdata_dates_MOD, na.action = na.exclude)

```

```{r crpmodel_logcr_int, echo=FALSE, include=TRUE}
# crp

tab_model(model_crp_logcr_INT,
  show.icc = TRUE,
  digits = 3, show.obs = TRUE, show.re.var = TRUE,
  title = paste(model_crp_logcr_INT@call$formula[3]))
# model has been inspected: interaction term p-value>0.05

```

```{r plots_model_crp_logcr_INT, echo=FALSE}

plot(model_crp_logcr_INT, resid(.) ~ fitted(.) | phase)

plot(model_crp_logcr_INT, resid(.) ~ DaysfromTreatment)

qqnorm(resid(model_crp_logcr_INT))

```


```{r  crpmodel_logcr_nonINT , echo=FALSE, include=FALSE}

# model without interaction term
model_crp_logcr_nonINT <- lmer(logcrp_S_TR ~ logcrp_S_BL + DaysfromTreatment + phase + logcr_S +(1 | ID), lmmdata_dates_MOD, na.action = na.exclude)

# summary(model_crp_logcr_nonINT)
coefs_crp_logcr_nonINT <- merge(
  as.data.frame(
    round(
      # getting from the model coefficients table
      # the fixed effects names --- row.names
      ...
      # the std. error --- col. 2
      # the p-value --- col. 5
      coef(summary(model_crp_logcr_nonINT))[, c(1, 2, 5)],
      digits = 3
    )
  ),
  as.data.frame(
    round(
      # getting the confidence intervals omitting the first 3 rows that
      # refer to sigma (it is NA in the Wald method)
      confint.merMod(model_crp_logcr_nonINT, method = "Wald"),
      digits = 3
    )[-c(1:3), ]
  ),
  by = "row.names"
...
# change the names in the columns of the CIs
setnames(coefs_crp_logcr_nonINT, old = c("Pr(>|t|)", "2.5 %", "97.5 %"), new = c("p", "CI2.5", "CI97.5"))

coefs_crp_logcr_nonINT$Model <- "crp_nonadjusted_nonINT"

# revalue the fixed effects names
coefs_crp_logcr_nonINT$FixedEffects <- revalue(
  coefs_crp_logcr_nonINT$Row.names,
  c(
    "(Intercept)" = "Intercept",
    "DaysfromTreatment" = "Time (days)",
    "phaseOrganic" = "Organic diet",
    "logcrp_S_BL" = "Baseline crp levels (no cr adjusted)",
    "logcr_S"="log_creatinine"  
  )
)

```

```{r tabmodel_logcr, echo=FALSE, include=TRUE}
# crp
tab_model(model_crp_logcr_nonINT,
  show.icc = TRUE,
  digits = 3, show.obs = TRUE, show.re.var = TRUE,
  title = paste(model_crp_logcr_nonINT@call$formula[3]))

```

```{r model_crp_logcr_nonINT, echo=FALSE}

plot(model_crp_logcr_nonINT, resid(.) ~ fitted(.) | phase)

plot(model_crp_logcr_nonINT, resid(.) ~ DaysfromTreatment)

qqnorm(resid(model_crp_logcr_nonINT))
...
### Log CRP ~ log baseline CRP + 6-CN + DaysfromTreatment + sex + baseline age + log-creatinine_s + random(...)

```{r add 6-CNlogcr non adjusted, echo=FALSE, include=TRUE}
# crp

Model2b_logcr<-lmer(logcrp_S_TR ~ logcrp_S_BL + cnlod + DaysfromTreatment + Sex + age_baseline_S + logcr_S + (1 | ID), lmmdata_dates_MOD, na.action = na.exclude)
```

```{r logcr_6cn, echo=FALSE, include=TRUE}

tab_model(Model2b_logcr,
  show.icc = TRUE,
  digits = 3, show.obs = TRUE, show.re.var = TRUE,
  title = paste(Model2b_logcr@call$formula[3])
)

```

```{r diagnostics_Model2b_logcr, echo=FALSE}

plot(Model2b_logcr, resid(.) ~ fitted(.) | phase)

plot(Model2b_logcr, resid(.) ~ DaysfromTreatment)

qqnorm(resid(Model2b_logcr))
...
### Log CRP ~ Log baseline CRP + 3-PBA + DaysfromTreatment + sex + baseline age + logcr_S + random(...)

```{r logcr_3-PBA_nonadjusted, echo=FALSE, include=TRUE}
# PBA

Model3b_logcr<-lmer(logcrp_S_TR ~ logcrp_S_BL + pb1000_S + DaysfromTreatment + Sex + age_baseline_S + logcr_S + (1 | ID), lmmdata_dates_MOD, na.action = na.exclude)

```

```{r logcr_3pba, echo=FALSE, include=TRUE}

tab_model(Model3b_logcr,
  show.icc = TRUE,
  digits = 3, show.obs = TRUE, show.re.var = TRUE,
  title = paste(Model3b_logcr@call$formula[3])
)

```

```{r diagnostics_Model3b_logcr, echo=FALSE}

plot(Model3b_logcr, resid(.) ~ fitted(.) | phase)

plot(Model3b_logcr, resid(.) ~ DaysfromTreatment)

qqnorm(resid(Model3b_logcr))
